...
app.get('/player_info/post', function(req, res){
  ...
  var query2 = 'SELECT * FROM football_players WHERE id = \'' + req.query.player_choice + '\';';
  var query3 = 'SELECT COUNT(*) FROM football_players INNER JOIN football_games ON football_players.id = ANY(football_games.players) WHERE id =\'' + req.query.player_choice + '\';';
  db.task('get-everything', task => {
      return task.batch([
          task.any(query1),
          task.any(query2),
  ...
  .then(info => {
    res.render('pages/player_info',{
        ...
        average_passing: parseInt(info[1][0].passing_yards) / parseInt(info[2][0].count),
        average_rushing: parseInt(info[1][0].rushing_yards) / parseInt(info[2][0].count),
        average_receiving: parseInt(info[1][0].receiving_yards) / parseInt(info[2][0].count),
        total_games: info[2][0].count
